...
public class Nametags extends Module {
    ...
    DValue range = setting("Range", 200,20,400d);
    DValue scale = setting("Scale",5,1,10d);
    BValue health = setting("Health", true);
    ...
    private void drawNametag(Entity entityIn) {
        ...
        String str = entityIn.getName() + (health.getValue() ? " " + ChatUtil.SECTIONSIGN + "c" + Math.round(((EntityLivingBase) entityIn).getHealth() + (entityIn instanceof EntityPlayer ? ((EntityPlayer) entityIn).getAbsorptionAmount() : 0)) : "");
        ...
        fontRendererIn.drawString(str, -i, 10, entityIn instanceof EntityPlayer ? FriendManager.isFriend(entityIn.getName()) ? 0x11ee11 : 0xffffff : 0xffffff);
