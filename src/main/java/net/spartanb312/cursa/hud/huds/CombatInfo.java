    ...
    public CombatInfo() {
        asyncRenderer = new AsyncRenderer() {
            ...
            public void onUpdate(ScaledResolution resolution, int mouseX, int mouseY) {
                ...
                int color = GUIManager.isRainbow() ? WordArt.rainbow(0) : GUIManager.getColor3I();
                drawAsyncRect(x, y, x + width, y + 3f, color);
                startY += 6;
                float spacing = FontRenderers.getHeight() * 0.8f * scale.getValue() * 0.8f;
                ...
                if (items.getValue()) {
                    if (goldenApple.getValue()) {
                        ...
                        maxWidth = Math.max(maxWidth, FontRenderers.getWidth(text));
