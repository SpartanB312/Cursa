...
class EntitySpeed : Module() {
    ...
    private val speed = setting("Speed", 1f, 0f, 50f)
    private val antiStuck = setting("AntiStuck", false)
    private val flight = setting("Flight", false)
    private val wobble = setting("Wobble", true).b(flight)
    ...
    private fun steerEntity(entity: Entity) {
        ...
        if (flight.value) {
            if (mc.gameSettings.keyBindJump.isKeyDown) entity.motionY =
                speed.value.toDouble() else if (mc.gameSettings.keyBindForward.isKeyDown || mc.gameSettings.keyBindBack.isKeyDown) entity.motionY =
                if (wobble.value) sin(
                    mc.player.ticksExisted.toDouble()
                ) else 0.0
        }
    ...
    private fun isBorderingChunk(entity: Entity, motX: Double, motZ: Double): Boolean {
        return antiStuck.value && mc.world.getChunk(
            (entity.posX + motX).toInt() shr 4,
            (entity.posZ + motZ).toInt() shr 4
        ) is EmptyChunk
    }
}